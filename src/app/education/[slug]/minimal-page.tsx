...
export default async function MinimalArticlePage({ params }: { params: { slug: string } }) {
  const { slug } = params
                      ...
                      {article.content.split('\n').map((line: string, index: number) => (
                        <div key={index} className="mb-2">
                          {line.startsWith('# ') ? (
                            ...
                            <h2 className="text-2xl font-bold text-white mb-3">{line.substring(3)}</h2>
                          ) : line.startsWith('### ') ? (
                            <h3 className="text-xl font-bold text-white mb-2">{line.substring(4)}</h3>
                          ) : line.startsWith('**') && line.endsWith('**') ? (
                            <p className="font-bold text-white mb-2">{line.substring(2, line.length - 2)}</p>
                          ) : line.trim() === '' ? (
                            <br />
                          ) : (
