...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const row = await fetchPublishedListingBySlug('presenter', params.slug)
  if (!row) return { title: 'Presenter Not Found' }
  return {
    title: row.name,
    description: row.description?.slice(0, 160) ?? `${row.name} on East Coast Kink Events.`,
    alternates: { canonical: `${BASE_URL}/presenters/${row.slug}` },
  }
}

export default async function PresenterListingPage({ params }: PageProps) {
  const row = await fetchPublishedListingBySlug('presenter', params.slug)
  if (!row) notFound()
  return (
    <KinkSocialListingDetailView
      entityLabel="Presenter"
