                ...
                {dungeon.images && dungeon.images.length > 0 && (
                  <div className="mt-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {dungeon.images.map((image: string, index: number) => (
                        <div key={index} className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-dark-900 to-black border border-dark-700 hover:border-primary-500/50 transition-all duration-300">
                          <div className="aspect-video relative">
                            <Image
                              src={image}
                              alt={`${dungeon.name} facility image ${index + 1}`}
                              fill
                          ...
                          <div className="absolute bottom-2 left-2 right-2 text-white text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/50 backdrop-blur-sm rounded px-2 py-1">
                            {image.includes('SocialSpace') ? 'Social Space' : 
                             image.includes('PlaySpace') ? 'Play Space' : 
                             image.includes('Splash') ? 'Facility Overview' : 
                             'Facility Image'} {index + 1}
                          </div>
                        </div>
                      ))}
                    </div>
