/**
 * US state SVG paths for the /states index map.
 *
 * Contiguous US + AK/HI insets use the commonly circulated Albers-style
 * public-domain path set derived from Wikimedia Commons
 * "Blank US Map (states only)" (CC0 1.0 Public Domain Dedication).
 * https://commons.wikimedia.org/wiki/File:Blank_US_Map_(states_only).svg
 * viewBox 0 0 959 593.
 *
 * DC is a simplified original inset marker. The compact path set has no
 * usable DC hit target. Coordinates are SVG-space only, not lat/lng.
 */
export const US_STATE_MAP_VIEWBOX = '0 0 959 593' as const

export type UsStateMapPath = {
  abbr: string
  d: string
  inset?: boolean
}

export type UsStateMapLabel = {
  x: number
  y: number
}

export const US_STATE_SVG_PATHS: UsStateMapPath[] = [
  {
    abbr: 'AK',
  ...
  {
    abbr: 'DC',
    d: "M808,255 m-7.5,0 a7.5,7.5 0 1,0 15,0 a7.5,7.5 0 1,0 -15,0",
    inset: true
  }
]

/** Decorative SVG-space labels for crowded Northeast states + DC. Not GPS. */
export const US_STATE_MAP_LABELS: Record<string, UsStateMapLabel> = {
  DC: {
    x: 826,
    y: 258
  },
