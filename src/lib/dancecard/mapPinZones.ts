...
export function normalizeMapZonePin(
  pin: MapZonePin,
): Required<Pick<MapZonePin, 'shape' | 'width' | 'height' | 'rotation'>> & MapZonePin {
  const shape = isMapZoneShape(pin.shape) ? pin.shape : 'circle'
