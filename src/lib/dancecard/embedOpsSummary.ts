...
export function buildOpsSummaryEmbedHtml(opts: {
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${esc(
    opts.title,
  )}</title><style>${rootStyle}body{font-family:system-ui,sans-serif;background:var(--dc-surface,#f4f0e8);color:var(--dc-text,#1c1814);margin:0;padding:12px}.ring{width:48px;height:48px;border-radius:50%;background:conic-gradient(var(--dc-accent,#8b6914) ${pct * 3.6}deg,var(--dc-surface-muted,#e8e2d6) 0);display:flex;align-items:center;justify-content:center}.ring span{width:38px;height:38px;border-radius:50%;background:var(--dc-elevated,#ffffff);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700}</style></head><body><div style="display:flex;gap:12px;align-items:center"><div class="ring"><span>${pct}%</span></div><div><p style="margin:0;font-size:14px;font-weight:600">${esc(opts.title)}</p><p style="margin:4px 0 0;font-size:12px;color:#4f483f">${opts.publishedCount}/${opts.totalSlots} sessions published · ${esc(opts.registrantBand)}</p><p style="margin:2px 0 0;font-size:11px;color:#6e5310">Integrations ${opts.integrationsOk ? '✓' : '—'}</p></div></div>${postMessage}</body></html>`
}
