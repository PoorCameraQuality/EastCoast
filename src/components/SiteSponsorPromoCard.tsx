...
function SponsorCta({ href, label, className }: { href: string; label: string; className: string }) {
  if (href.startsWith('/')) {
    return (
...
/** Featured site sponsor card — events/orgs supporting ECKE. */
export default function SiteSponsorPromoCard({ promo, variant = 'compact' }: Props) {
  const isWide = variant === 'wide'
  const ctaClass =
    'site-sponsor-promo-cta inline-flex min-h-touch items-center justify-center rounded-xl px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-black sm:text-sm'
  const eventHref = promo.websiteUrl || promo.href

  return (
    <div
      className={`relative overflow-hidden rounded-2xl border border-amber-300/40 bg-gradient-to-br from-black via-slate-950 to-amber-950/30 site-sponsor-promo ${
    ...
    >
      <div className={`relative z-[1] flex flex-col ${isWide ? 'gap-5' : 'gap-3'}`}>
        ...
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-[0.18em] text-amber-200/90">{promo.eyebrow}</p>
          <h3
            className={`font-serif font-semibold text-white mt-1 ${isWide ? 'text-2xl sm:text-3xl' : 'text-base sm:text-lg'}`}
          >
            {promo.headline}
          </h3>
          ...
          <p className={`text-gray-400 mt-2 ${isWide ? 'text-xs' : 'text-[10px] sm:text-xs'}`}>
            Helps keep East Coast Kink Events online as a public discovery guide.
          </p>
          <div className={`mt-4 flex flex-col gap-2 ${isWide ? 'sm:flex-row sm:flex-wrap' : ''}`}>
            <SponsorCta href={promo.href} label={promo.ctaLabel} className={ctaClass} />
            <SponsorCta
              href={eventHref}
