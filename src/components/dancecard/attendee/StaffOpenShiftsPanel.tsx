...
export function StaffOpenShiftsPanel({
  ...
  return (
    <section className="rounded-xl border border-violet-400/25 bg-violet-100 p-4 text-sm text-dc-text">
      ...
      {!shifts.length ? (
        ...
        <ul className="mt-3 space-y-2">
          {shifts.map((s) => (
            <li
              key={s.id}
              className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-dc-border bg-dc-elevated-muted px-3 py-2"
            >
              ...
              <button
                type="button"
                disabled={busy === s.id}
                className="rounded-full bg-violet-600/80 px-3 py-1 text-[10px] font-semibold text-dc-text disabled:opacity-40"
