...
export default function DancecardAppearancePicker({ className = '', compact = false }: Props) {
  ...
  return (
    <label
      ...
      title="Comfort themes for Dancecard and organizer — saved on this device"
    >
      {!compact ? (
        <span className="shrink-0 text-dc-micro uppercase tracking-wide text-dc-muted">Theme</span>
      ) : null}
      <select
        className="min-w-0 max-w-full truncate rounded-lg border border-dc-border bg-dc-elevated-solid px-2 py-1.5 text-sm text-dc-text"
        value={appearanceId}
        onChange={(e) => setAppearanceId(e.target.value as DancecardAppearanceId)}
        aria-label="Dancecard appearance theme"
      >
        {presets.map((p) => (
