...
export function IcalBusyPreviewPanel({ eventSlug }: { eventSlug: string }) {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-black/30 p-4">
      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">ICS busy preview</p>
      <p className="mt-1 text-xs text-slate-400">
        Paste exported calendar (.ics) text to see busy blocks before importing or comparing schedules.
      </p>
      <textarea
        className="mt-3 min-h-[120px] w-full rounded-xl border border-white/10 bg-[#111a2c] px-3 py-2 font-mono text-xs text-white"
        value={icsText}
      ...
      {blocks.length ? (
        <div className="mt-3 max-h-48 overflow-y-auto">
          <p className="text-xs text-slate-400">{blocks.length} block(s)</p>
          <ul className="mt-2 space-y-1 text-xs text-slate-300">
            {blocks.map((b, i) => (
              <li key={`${b.start}-${i}`} className="rounded-lg border border-white/5 bg-white/[0.03] px-2 py-1.5">
                <span className="text-slate-400">{b.start}</span>
                <span className="mx-1">→</span>
                <span className="text-slate-400">{b.end}</span>
                {b.summary ? <span className="ml-2 text-white">{b.summary}</span> : null}
              </li>
            ))}
          </ul>
        </div>
