...
import { toLocalDatetimeInput } from '@/components/dancecard/organizer/settings/EventSettingsEventDto'
  ...
  return (
    <>
      ...
      <DatetimeLocalField
        variant="settings"
        label="Event starts"
        value={toLocalDatetimeInput(event.windowStartsAt)}
        disabled={!canEdit}
        hint=""
        onChange={(v) =>
          setEvent((ev) => (ev ? { ...ev, windowStartsAt: new Date(v).toISOString() } : ev))
        }
      ...
      <DatetimeLocalField
        variant="settings"
        label="Event ends"
        value={toLocalDatetimeInput(event.windowEndsAt)}
        disabled={!canEdit}
        hint=""
        onChange={(v) =>
          setEvent((ev) => (ev ? { ...ev, windowEndsAt: new Date(v).toISOString() } : ev))
        }
