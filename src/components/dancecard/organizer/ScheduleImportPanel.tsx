...
function makeLocalDemoRows(kind: ImportKind, windowStartsAt: string): ImportRow[] {
  if (kind === 'staff') {
...
export function ScheduleImportPanel({
  ...
  const canConfigureGoogle = organizerRole === 'owner' || organizerRole === 'admin'
  const [kind, setKind] = useState<ImportKind>('program')
  ...
  async function uploadImport() {
    ...
    setMessage(null)
    try {
      let response: { batch: ImportBatch; rows: ImportRow[] }
      if (file) {
        const form = new FormData()
        form.set('kind', kind)
        form.set('file', file)
        const res = await fetch(`/api/organizer/dancecard/${encodeURIComponent(eventSlug)}/imports`, {
          method: 'POST',
          credentials: 'include',
          body: form,
        })
        ...
        const rows = jsonText.trim() ? JSON.parse(jsonText) : []
        response = await organizerDancecardFetch(eventSlug, '/imports', {
          method: 'POST',
      ...
      await loadLocations()
      setMessage(`Imported ${response.batch.summary.total ?? response.rows.length} draft rows.`)
    } catch (e) {
  ...
  async function patchRow(rowId: string, patch: Record<string, unknown>) {
    ...
    try {
      ...
      setRows((current) => current.map((row) => (row.id === rowId ? res.row : row)))
    } catch (e) {
  ...
  async function publishBatch() {
    ...
    if (
      !(await ask({
        title: 'Publish draft schedule?',
        message: 'Publish this draft schedule? Affected attendees may receive change notices.',
      }))
    )
      return
    setBusy(true)
    setErr(null)
    try {
      const res = await organizerDancecardFetch<{ summary: { added: number; updated: number; skipped: number; notified: number } }>(
        eventSlug,
        `/imports/${batch.id}/publish`,
        { method: 'POST' }
      )
      setBatch({ ...batch, status: 'published', summary: { ...batch.summary, ...res.summary } })
      setMessage(`Published. Added ${res.summary.added}, updated ${res.summary.updated}, notified ${res.summary.notified}.`)
    } catch (e) {
  ...
  const importStep: 1 | 2 | 3 = !batch ? 1 : batch.status === 'published' ? 3 : 2

  return (
    <div className="space-y-5">
      <Panel variant="default" className="border border-dc-accent-border bg-dc-accent-muted p-4">
        <p className="text-dc-micro font-semibold uppercase tracking-[0.22em] text-dc-text">Import runbook</p>
        <h3 className="mt-2 font-serif text-lg text-dc-text">First-time schedule import</h3>
        <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm leading-relaxed text-dc-muted">
          <li>Set event dates and timezone under Settings before importing.</li>
          <li>Choose program vs staff import so it matches your spreadsheet.</li>
          <li>
            Connect Google Sheets below (recommended) or upload a CSV/XLSX export from your workbook.
          </li>
          <li>Preview rows on the draft board; publish only when everything looks correct.</li>
        </ol>
      ...
      <div className="rounded-3xl border border-dc-border bg-dc-elevated-solid p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-dc-muted">
              Upload a class or staff spreadsheet, review rows on the draft board, then publish when it looks right.
            </p>
          ...
          <div className="flex flex-wrap gap-2">
            {(['program', 'staff'] as const).map((option) => (
              <button
                key={option}
                type="button"
                className={cx(
        ...
        <ol className="mt-4 flex flex-wrap gap-2 text-dc-micro font-semibold uppercase tracking-wide">
          {[
            ...
            { n: 3, label: 'Import to live schedule' },
          ].map((step) => (
            <li
              key={step.n}
              className={cx(
        ...
        <GoogleSheetsImportSection
          ...
          onImportLoaded={(loadedBatch, loadedRows) => {
            setBatch(loadedBatch as ImportBatch)
            setRows(loadedRows as ImportRow[])
            void loadLocations()
          }}
          onClearFileUpload={() => {
            setFile(null)
          ...
          <label className="block text-xs font-semibold uppercase tracking-[0.18em] text-dc-muted">
            ...
            <input
              ...
              onChange={(event) => {
                ...
                setFile(f)
                setJsonText('')
                if (f && /staff|volunteer|schedule/i.test(f.name)) {
                  setKind('staff')
                }
              }}
            />
            <span className="mt-2 block text-xs font-normal normal-case leading-relaxed text-dc-muted">
              Staff/volunteer grid spreadsheets (one sheet per day, names in cells) use the <strong>staff</strong> import
