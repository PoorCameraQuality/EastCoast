  ...
  if (!maps.length) {
    return (
      <Panel variant="muted" className="space-y-2 text-sm text-dc-muted">
        ...
        {onGoVenueSettings ? (
          <button type="button" className="text-dc-accent hover:underline" onClick={onGoVenueSettings}>
            Upload a map in Settings → Rooms & maps
          </button>
        ) : (
          <Link className="text-dc-accent hover:underline" href={`?tab=settings&settingsPanel=venue`}>
            Upload a map in Settings → Rooms & maps
          </Link>
        )}
      </Panel>
    ...
    <div className="flex flex-col gap-3">
      <div>
        ...
        <p className="mt-1 text-xs text-dc-muted">
          Drag classes onto a room pin, or use the time grid. Pins come from Settings → Rooms & maps.
        </p>
      </div>
      ...
      {activeMap?.imageUrl ? (
        pinsWithCoords.length > 0 ? (
          ...
          <Panel variant="muted" className="text-sm text-dc-muted">
            ...
            {onGoVenueSettings ? (
              <button type="button" className="mt-2 text-dc-accent hover:underline" onClick={onGoVenueSettings}>
        ...
        <Panel variant="muted" className="text-sm text-dc-muted">
          Map image unavailable. Re-upload in settings.
        </Panel>
      )}
    </div>
  )
