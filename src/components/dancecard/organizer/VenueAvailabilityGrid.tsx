    ...
    async function loadCoverage() {
      if (!linkedLeafIds.size) {
        if (!cancelled) {
          setCoverage({
            tone: 'info',
            title: 'Add named rooms in settings to track monitor coverage on this grid.',
          })
        }
        return
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="font-serif text-2xl text-dc-text">Rooms by time</h2>
      ...
      {leafColumns.length === 0 ? (
        ...
        <aside className="xl:sticky xl:top-4 xl:self-start">
          <VenueMapAssignPanel
            ...
            onGoVenueSettings={onGoVenueSettings}
          />
        </aside>
      </div>
    </div>
