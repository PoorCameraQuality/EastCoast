    ...
    <div className="rounded-lg border border-slate-800 bg-[#0a1322] p-2 sm:rounded-xl sm:p-2.5">
      ...
      >
        {hourColumns.map(({ hour, slots: pair }) => (
          <div
            key={hour}
            className="flex min-h-11 min-w-0 w-full gap-px border-l border-white/15 first:border-l-0"
          >
            {pair.map((slot, j) => {
              ...
              if (free && slot.inPlayableWindow && onFreeStepClick) {
                return (
                  <button
                    ...
                    className={cx(
                      'relative z-[1] min-h-11 min-w-[12px] w-full flex-1 cursor-pointer touch-manipulation rounded-sm bg-emerald-500/85 transition hover:bg-emerald-400 hover:ring-2 hover:ring-emerald-200/50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-cyan-300'
                    )}
                  />
                )
              }
