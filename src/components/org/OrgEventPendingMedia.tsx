...
export const EMPTY_PENDING_MEDIA: PendingEventMedia = {
  hero: null,
  logo: null,
  gallery: [],
  program: null,
...
function validateFile(kind: EventAssetKind, file: File): string | null {
  const guide = EVENT_ASSET_GUIDES[kind]
  if (file.size > guide.maxBytes) return `File is too large. ${guide.hint}`
  return null
}

async function uploadAsset(slug: string, kind: EventAssetKind, file: File) {
...
export async function uploadPendingEventMedia(slug: string, pending: PendingEventMedia) {
  const failed: string[] = []
  const jobs: Array<{ kind: EventAssetKind; file: File }> = []
  if (pending.hero) jobs.push({ kind: 'hero', file: pending.hero })
  if (pending.logo) jobs.push({ kind: 'logo', file: pending.logo })
  if (pending.program) jobs.push({ kind: 'program', file: pending.program })
  if (pending.map) jobs.push({ kind: 'map', file: pending.map })
  for (const file of pending.gallery) jobs.push({ kind: 'gallery', file })

  for (const job of jobs) {
    try {
      await uploadAsset(slug, job.kind, job.file)
    } catch (error) {
...
export default function OrgEventPendingMedia({
  value,
  onChange,
}: {
  value: PendingEventMedia
  onChange: (next: PendingEventMedia) => void
}) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-sf-muted">
        Optional. Choose files now and they upload when you create the event. You can also add or replace them later
        from Media.
      </p>
      <FileSlot
        kind="hero"
      ...
      <FileSlot
        kind="program"
        file={value.program}
        onPick={(file) => onChange({ ...value, program: file })}
        onClear={() => onChange({ ...value, program: null })}
      />
      <FileSlot
        kind="map"
        file={value.map}
        onPick={(file) => onChange({ ...value, map: file })}
        onClear={() => onChange({ ...value, map: null })}
      />
    </div>
  )
}
