-- =====================================================
-- COMPREHENSIVE ARTICLE NORMALIZATION SQL
-- =====================================================
-- This SQL will clean up all articles in your database
-- Run this in your Supabase SQL Editor

-- 1. First, let's see what articles we have
SELECT 
    id,
    title,
    slug,
    ...
    CASE 
        WHEN content LIKE '%—%' OR content LIKE '%–%' THEN 'HAS_EM_DASHES'
        WHEN content LIKE '%|%' THEN 'HAS_TABLES'
        WHEN content LIKE '%## %' THEN 'HAS_MARKDOWN'
        ELSE 'STANDARD'
    END as content_type
FROM articles 
ORDER BY publish_date DESC;

-- 2. Update articles with em/en dashes
UPDATE articles 
SET content = REPLACE(REPLACE(content, '—', ' - '), '–', ' - ')
WHERE content LIKE '%—%' OR content LIKE '%–%';

-- 3. Fix table formatting - add separators where missing
-- This is a more complex operation that requires careful handling
-- We'll do this step by step

-- First, let's identify articles with tables that might need fixing
SELECT 
    id,
    title,
    slug,
    CASE 
        WHEN content LIKE '%|%' AND content NOT LIKE '%|---%' THEN 'NEEDS_TABLE_SEPARATOR'
        WHEN content LIKE '%|%' THEN 'HAS_TABLES'
        ELSE 'NO_TABLES'
    END as table_status
FROM articles 
...
-- 4. For the specific research article, let's apply the complete fix
-- (This is the one we know has table issues)
UPDATE articles 
SET content = $$
...
SELECT 
    'Articles with em/en dashes fixed' as change_type,
    COUNT(*) as count
FROM articles 
WHERE content LIKE '% - %' AND (content LIKE '%—%' = false AND content LIKE '%–%' = false)

UNION ALL

SELECT 
    'Articles with tables' as change_type,
    COUNT(*) as count
FROM articles 
WHERE content LIKE '%|%'
